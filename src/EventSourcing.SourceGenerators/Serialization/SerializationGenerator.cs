...
public class SerializationGenerator : IIncrementalGenerator
{
    ...
    private static AggregateInfo? GetRepositoriesAggregateInfo(GeneratorSyntaxContext context)
    {
        ...
        // Analyze aggregateType for Apply methods
        var applyMethods = aggregateType.GetMembers()
            ...
            .Select(m =>
            {
                ...
                return new MutateMethodInfo
                {
                    ...
                    EventName = eventType.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat),
                    SaveEventName = eventType.Name,
        ...
        // Analyze baseType hierarchie of aggregateType for Apply methods
        var currentBaseType = aggregateType.BaseType;
        ...
        // Analyze aggregateType for Create methods
        ...
        // Analyze baseType hierarchie of aggregateType for Create methods
        currentBaseType = aggregateType.BaseType;
        while (currentBaseType != null)
        {
            var baseApplyMethods = currentBaseType.GetMembers()
                .OfType<IMethodSymbol>()
    ...
    private static MapperInfo[] CreateMapperInfos(AggregateInfo info)
    {
        ...
        foreach (var evt in events)
            mapperDataList.Add(new MapperInfo
            {
                ...
                EventKebabCaseName = ToKebabCase(evt.SaveEventName),
                EventVariableName = $"{char.ToLower(evt.SaveEventName[0]) + evt.SaveEventName.Substring(1)}",
                EventFieldName = $"_{char.ToLower(evt.SaveEventName[0]) + evt.SaveEventName.Substring(1)}",
                MapperName = $"{info.AggregateName}{evt.SaveEventName}Mapper",
                MapperFullname = $"{info.RepositoryNamespace}.{info.AggregateName}{evt.SaveEventName}Mapper",
                MapperNamespace = info.RepositoryNamespace,
                MapperFieldName = $"_{char.ToLower(evt.SaveEventName[0]) + evt.SaveEventName.Substring(1)}Mapper"
            });
    ...
    private static string CreateSerializationRegistrySource(AggregateInfo aggregateInfo, ImmutableArray<MapperInfo> mapperInfos)
    {
        ...
        foreach (var mapperInfo in mapperInfos)
            sb.AppendLine($"            {mapperInfo.EventName} {mapperInfo.EventVariableName} when @event.GetType() == typeof({mapperInfo.EventName}) => Serialize({mapperInfo.EventVariableName}),");

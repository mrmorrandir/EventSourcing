...
public interface IRepository<TAggregate> where TAggregate : IAggregate
{
    ...
    Task<Result<TAggregate>> CreateAsync<TEvent>(Func<Task<Result<TEvent>>> create, CancellationToken cancellationToken = default) where TEvent : IEvent;
    Task<Result<TAggregate>> CreateAsync<TEvent>(Func<Result<TEvent>> create, CancellationToken cancellationToken = default) where TEvent : IEvent;
    ...
    Task<Result<TAggregate>> UpdateAsync(Guid aggregateId, Func<TAggregate, Task<Result<List<IEvent>>>> update, CancellationToken cancellationToken = default);
    Task<Result<TAggregate>> UpdateAsync(Guid aggregateId, Func<TAggregate, Result<List<IEvent>>> update, CancellationToken cancellationToken = default);
    Task<Result<TAggregate>> UpdateAsync<TEvent>(Guid aggregateId, Func<TAggregate, Task<Result<TEvent>>> update, CancellationToken cancellationToken = default) where TEvent : IEvent;
    Task<Result<TAggregate>> UpdateAsync<TEvent>(Guid aggregateId, Func<TAggregate, Result<TEvent>> update, CancellationToken cancellationToken = default) where TEvent : IEvent;
}
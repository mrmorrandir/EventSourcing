...
public class DependencyInjectionTests
{
    ...
    public void RegistrationShouldWork_WhenEventMappersAndEventsAreCorrect()
    {
        ...
        var eventMappers = provider.GetRequiredService<IEnumerable<IEventMapper>>();
        
        eventMappers.Should().ContainSingle(m => m.Types.Contains("my-custom-event") && m.EventType == typeof(ValidAssembly.CustomEvent));
        eventMappers.Should().ContainSingle(m => m.Types.Contains("default-event") && m.EventType == typeof(ValidAssembly.DefaultEvent));
